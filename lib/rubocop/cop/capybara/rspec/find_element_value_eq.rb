  ...
  module Cop
    module Capybara
      module RSpec
        ...
        #
        class FindElementValueEq < RuboCop::Cop::Base # rubocop:disable Metrics/ClassLength
          ...
          def register_offense(node, finder, to_symbol, matcher, expected)
            ...
            return add_offense(node.loc.selector) unless matcher_replacement
          ...
          def replacement_matcher(to_symbol, expected, field_locator)
            expected_value = replacement_value(expected)
            return unless expected_value

            matcher = to_symbol == :to ? 'have_field' : 'have_no_field'
            args = ["with: #{expected_value}"]
            args.unshift(replacement_locator(field_locator)) if field_locator

            "#{matcher}(#{args.join(', ')})"
          end

          def replacement_locator(locator)
            return locator.source if locator.hash_type? && !locator.braces?

            replacement_value(locator)
          end

          def replacement_value(expected)
            return if unsafe_expected_value?(expected)

            if expected.hash_type? && !expected.braces?
          ...
          def unsafe_expected_value?(node)
            node.splat_type? ||
              (node.hash_type? && node.children.any?(&:kwsplat_type?))
          end
