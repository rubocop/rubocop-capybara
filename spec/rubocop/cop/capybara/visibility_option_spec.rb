...
RSpec.describe RuboCop::Cop::Capybara::VisibilityOption do
  ...
  it 'registers offenses for `within`' do
    expect_offense(<<~RUBY)
      within('.menu', visible: true) {}
                      ^^^^^^^^^^^^^ Use `:visible` instead of `true`.
    RUBY

    expect_correction(<<~RUBY)
      within('.menu', visible: :visible) {}
    RUBY
  end
  ...
  it 'does not duplicate the RSpec visibility matcher offenses' do
    expect_no_offenses(<<~RUBY)
      expect(page).to have_css('.menu', visible: false)
      expect(page).to have_no_css('.menu', visible: true)
      expect(page).to have_selector('.menu', visible: false)
    RUBY
  end
  ...
  it 'registers offenses through collection receivers' do
    expect_offense(<<~RUBY)
      page.all('.menus').first.find('.item', visible: true)
      #{' ' * 39}^^^^^^^^^^^^^ Use `:visible` instead of `true`.
      page.find_all('.menus').first.find('.item', visible: false)
      #{' ' * 44}^^^^^^^^^^^^^^ Use `:all` or `:hidden` instead of `false`.
    RUBY

    expect_correction(<<~RUBY)
      page.all('.menus').first.find('.item', visible: :visible)
      page.find_all('.menus').first.find('.item', visible: false)
    RUBY
  end

  it 'registers offenses through local variables' do
    expect_offense(<<~RUBY)
      element = find('.row')
      element.find('.item', visible: true)
      #{' ' * 22}^^^^^^^^^^^^^ Use `:visible` instead of `true`.
    RUBY

    expect_correction(<<~RUBY)
      element = find('.row')
      element.find('.item', visible: :visible)
    RUBY
  end
