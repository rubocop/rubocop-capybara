...
RSpec.describe RuboCop::Cop::Capybara::RSpec::FindElementValueEq do
  ...
  it 'preserves keyword options when using `find_field` value with `eq`' do
    expect_offense(<<~RUBY)
      expect(find_field(name: 'Email').value).to eq('user@example.com')
      ^^^^^^ Prefer `have_field` over checking `find(...).value`.
    RUBY

    expect_correction(<<~RUBY)
      expect(page).to have_field(name: 'Email', with: 'user@example.com')
    RUBY
  end
  ...
  it 'wraps keyword hash expected values during autocorrection' do
    expect_offense(<<~RUBY)
      expect(find('input').value).to eq(data: :value)
      ^^^^^^ Prefer `have_field` over checking `find(...).value`.
    RUBY

    expect_correction(<<~RUBY)
      expect(page).to have_field(with: {data: :value})
    RUBY
  end

  it 'does not autocorrect splatted expected values' do
    expect_offense(<<~RUBY)
      expect(find('input').value).to eq(*values)
      ^^^^^^ Prefer `have_field` over checking `find(...).value`.
      expect(find('input').value).to eq(**options)
      ^^^^^^ Prefer `have_field` over checking `find(...).value`.
    RUBY
