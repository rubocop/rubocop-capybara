...
RSpec.describe 'config/default.yml' do
  ...
  let(:cop_names) do
    glob = SpecHelper::ROOT.join('lib', 'rubocop', 'cop', 'capybara', '*.rb')
  ...
  it 'sorts configuration keys alphabetically with nested namespaces last' do
    keys = default_config.keys.select { |key| key.start_with?('Capybara') }
    namespaced_keys = keys.select do |key|
      key.start_with?(*(namespaces.values - ['Capybara']))
    end
